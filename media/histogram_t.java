import processing.core.*; import java.applet.*; import java.awt.*; import java.awt.image.*; import java.awt.event.*; import java.io.*; import java.net.*; import java.text.*; import java.util.*; import java.util.zip.*; public class histogram_t extends PApplet {
int D1, D2;
int X, Y;
int c1x, c1y, c2x, c2y;
...
public void draw() {
  ...
  for (int i = 0; i < steps; i++) { 
    float t = i / PApplet.toFloat(steps); 
  ...
  for (int i = DOTS/3; i < 2*steps; i++) { 
    float t = (i - DOTS/3) / PApplet.toFloat(steps); 
  ...
  for (int i = 2*DOTS/3; i < 3*steps; i++) { 
    float t = (i - 2*DOTS/3) / PApplet.toFloat(steps); 
  ...
  for (int i=0; i<width; i++) { 
    for (int j=0; j<height; j++) { 
      int grigio = PApplet.toInt(constrain(red(a.get(i, j)), 0, grayValues-1));
      int xii = xi[grigio]; 
      // int c = constrain(int(map[xii]) + int(random(10)) - 5, 0, grayValues-1);
      int c = constrain(PApplet.toInt(map[xii]), 0, grayValues-1);
